    ...
    if reactant == '->':
      ...
      for i in range(word_length):
        if   reactant[i] == '^' and reactant[i+2] == '-':
          ...
          reaction_string = reaction_string + 's'
        elif reactant[i] == '{':
          continue
        elif reactant[i] == '}':
          continue
  ...
  for rate in reaction_rates:
    ...
    for i in range(word_length):
      if rate[i] == 'x' and rate[i+1] == '1':                      # x10^{#}
        ...
        reaction_rate_string = reaction_rate_string + '(plas.Te/(Tg*k_to_v))'
      elif rate[i] == 'T' and rate[i+1] == '_' and rate[i+2] == '0': #T_0
        reaction_rate_string = reaction_rate_string + '(Tg/(300[K]))'
      elif rate[i] == 'T' and rate[i+1] == 'e':                      #Te
        reaction_rate_string = reaction_rate_string + '(plas.Te/(1.0[V]))'
      elif rate[i] == 'T' and rate[i+1] == 'g':                      #Tg
        reaction_rate_string = reaction_rate_string + '(Tg/(1[K]))'
      elif rate[i] == '_':
        continue
...
fid = open('rate_file.txt', 'w')
fid.write('Reaction list contain all reactions with:\n')
for i in pls_include:
  fid.write(i+', ')
  fid.write('\n')
fid.write('\n\n')
for i in range(valid_reaction_number):
  fid.write('Reaction #'+str(i)+'\n')
