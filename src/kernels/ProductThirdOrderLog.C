...
ProductThirdOrderLog::computeQpResidual()
{
  ...
  {
    return -_test[_i][_qp] * (_stoichiometric_coeff) * _reaction_coeff[_qp] * std::exp(_v[_qp]) * getMaterialProperty<Real>("n_gas")[_qp] * std::exp(_x[_qp]);
  }
  else if (!isCoupled("v") && !isCoupled("w") && isCoupled("x"))
  {
    return -_test[_i][_qp] * (_stoichiometric_coeff) * _reaction_coeff[_qp] * std::exp(_w[_qp]) * getMaterialProperty<Real>("n_gas")[_qp];
  }
  else
  {
...
ProductThirdOrderLog::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  else if (!isCoupled("w") && isCoupled("v") && isCoupled("x"))
  {
    if (jvar == _v_id)
      return -_test[_i][_qp] * (_stoichiometric_coeff) * _reaction_coeff[_qp] *
              getMaterialProperty<Real>("n_gas")[_qp] * _phi[_j][_qp];
    else
      return 0.0;
  }
  else if (isCoupled("w") && !isCoupled("v") && isCoupled("x"))
