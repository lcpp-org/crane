...
DataRead::DataRead(const InputParameters & parameters)
  : AuxKernel(parameters),
    _sampler_var(coupledValue("sampler")),
...
Real
DataRead::computeValue()
{
  Real val;
  if (isCoupled("sampler"))
    val = _coefficient_interpolation.sample(_sampler_var[_qp]);
  else if (!isCoupled("sampler") && _use_time)
